...
function CtrlBtn({ onClick, title, children, disabled }: CtrlBtnProps): React.ReactElement {
  return (
    <button onClick={onClick} title={title} disabled={disabled} style={{
      ...
      color: disabled ? 'var(--border-lt)' : 'var(--tx-med)',
...
function EmptyState({ icon, message, color }: EmptyStateProps): React.ReactElement {
  return (
    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, color: color || 'var(--tx-lo)' }}>
...
export default function SessionReplay(): React.ReactElement {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-base)', color: 'var(--tx-hi)', overflow: 'hidden' }}>
      <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--tx-med)' }}>
          Date
          <input type="date" value={date} onChange={e => setDate(e.target.value)} style={{
            background: 'var(--bg-surface)', border: '1px solid var(--border-lt)', color: 'var(--tx-hi)',
        ...
        <button onClick={loadHistory} disabled={loading || !date} style={{
          ...
          color: loading ? 'var(--tx-dim)' : '#bfdbfe',
        ...
        {hasSnapshots && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--tx-lo)', marginLeft: 6 }}>{cursor + 1} / {snapshots!.length}</span>
          </div>
        )}
        {timestampStr && <span style={{ fontSize: 12, color: 'var(--tx-dim)' }}>{timestampStr}</span>}
      </div>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        {hasSnapshots && (
          <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} nodeTypes={nodeTypes} fitView colorMode={theme}>
